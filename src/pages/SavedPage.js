...
function SavedPage() {
    const {saved, removeFromSaved, emptySaved} = useSavedList()
        ...
        <div>
            <Header />
            {
               ...
               {saved.map(item =>
                   <Box key={item.id} >
                       <Card style={{display:"flex", justifyContent:"space-around", alignItems:"center", minWidth:"550px", maxWidth:"1000px", marginBottom:"20px", padding:"15px 0"}}>
                           ...
                           <DeleteIcon aria-label="delete" size="big" style={{color:"red", }} onClick={()=>removeFromSaved(item.id)} />
                       </Card>
                   </Box>
               )}
               <Box mt={6} style={{display:"flex", flexDirection:"column", alignItems:"center"}}>
                   ...
                   <Box mb={2} mt={4}>
                       <Button variant="contained" style={{backgroundColor:"black"}} onClick={emptySaved} > <ReplayIcon style={{width:"20px", marginRight:"5px", backgroundColor:"black"}} /> Delete all saved items</Button>
                   </Box>
                   <Box mb={2}>
